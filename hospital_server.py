...
def handle_client_connection(new_fd, addr):
    ...
    elif client_msg.split(',')[0] == "VIEW_APPT":
        tcp_port = new_fd.getsockname()[1]
        print(f"Hospital server has received a view appointment request from a user with hash suffix {client_msg.split(',')[1][-5:]} to view their appointment details using TCP over port {tcp_port}.")
        message = client_msg
    ...
    elif client_msg.split(',')[0] == "VIEW_APPTS":
        tcp_port = new_fd.getsockname()[1]
        print(f"Hospital Server has received a view appointments request from {client_msg.split(',')[2]} to view their schedule details using TCP over port {tcp_port}.")
        message = client_msg
